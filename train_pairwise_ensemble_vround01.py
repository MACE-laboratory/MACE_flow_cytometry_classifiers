...
def load_and_combine_data():
    ...
    # keep only monoculture files by community
    ...
    # NOTE: these dummies are NOT used in your current feature engineering because you later select numeric cols only.
    # Keeping them is harmless but doesn’t affect model features.
...
def save_pair_models(cls1, cls2, pair_result, models_root="round01_models"):
...
def main():
    ...
    # Save inference-critical artifacts
    joblib.dump(scaler, os.path.join(models_root, "round01_scaler.pkl"))
    joblib.dump(log_offsets, os.path.join(models_root, "round01_log_offsets.pkl"))
    joblib.dump(numeric_cols, os.path.join(models_root, "round01_numeric_cols.pkl"))
    joblib.dump(engineered_cols, os.path.join(models_root, "round01_engineered_feature_columns.pkl"))
    ...
    pd.DataFrame(all_stats).to_csv("all_pairwise_stats_round01.csv", index=False)
    print("✓ Training complete. Saved models + inference artifacts to round01_models/")
